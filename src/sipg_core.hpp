...
// Core data structures, platform includes, sparse matrix, and low-level utilities.
...
// Positive flux is prescribed inward, matching COMSOL's General inward heat flux q0.
struct HeatFluxCondition {
    int subdomain = -1;
    int boundaryEntity = -1;
    double inwardFluxWm2 = 0.0;
};
...
struct CaseConfig {
    ...
    std::vector<HeatFluxCondition> heatFluxConditions;
