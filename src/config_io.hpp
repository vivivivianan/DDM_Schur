    ...
    for (const std::string& rawLine : lines) {
        ...
        if (key == "name") {
            ...
            if (config.timeIntegrator != "backward_euler"
                ...
                throw std::runtime_error("time_integrator must be backward_euler or crank_nicolson.");
            }
        ...
        } else if (key == "heat_flux" || key == "neumann") {
            const std::vector<std::string> items = splitCsv(value);
            if (items.size() < 3) {
                throw std::runtime_error("heat_flux entry format: subdomain,boundary_entity,inward_flux_W_m2.");
            }
            config.heatFluxConditions.push_back(
                {parseSelectorInt(items[0]), std::stoi(items[1]), std::stod(items[2])});
