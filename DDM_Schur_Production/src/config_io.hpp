...
// Case configuration parsing and material lookup helpers. The parser resolves
// mesh paths relative to the case file, validates domain/interface references,
// and retains deterministic declaration order for cache fingerprints.
    ...
    for (const std::string& rawLine : lines) {
        ...
        if (key == "name") {
            ...
            if (config.timeIntegrator != "backward_euler") {
                throw std::runtime_error("time_integrator currently supports backward_euler only.");
            }
