...
// Workflow option schema inherited from the research executable. main.cpp is
// the production policy boundary and fixes every method-changing field; users
// can vary paths, steps, dt, threads, and optional final FOM validation only.
