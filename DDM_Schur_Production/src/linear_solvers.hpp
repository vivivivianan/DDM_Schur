...
// Sparse system utilities and MKL PARDISO ownership. Production Dynamic Schur
// uses SubdomainDirectSolver's symmetric-upper CSR constructor and reuses its
// symbolic/numerical factor for phase-33 solves; broader legacy helpers remain
// header-local dependencies of descriptor/FOM validation code.
...
static void buildSystemMatrix(const SparseMatrix& mass, const SparseMatrix& stiffness, double dt, SparseMatrix& system)
{
    ...
    system.appendScaledEntries(stiffness, 1.0);
}
...
public:
    ...
    // Solve several right-hand sides with one PARDISO phase-33 call.  The
    // dense RHS and solution arrays use the column-major n-by-nrhs layout
                       ...
                       int mklThreads = 1)
    {
        if (rightHandSides <= 0
            || rhs.size() != static_cast<size_t>(n_)
                * static_cast<size_t>(rightHandSides)) {
            throw std::runtime_error("PARDISO multi-RHS dimensions are invalid.");
        }
#ifdef USE_MKL_PARDISO
        if (!numericallyFactorized_) {
            throw std::runtime_error("PARDISO solve requested before numerical factorization.");
        ...
        if (error != 0) {
            throw std::runtime_error(
                "PARDISO multi-RHS solve failed with error " + std::to_string(error));
        }
#else
        x.assign(rhs.size(), 0.0);
        std::vector<double> columnRhs(static_cast<size_t>(n_), 0.0);
        std::vector<double> columnSolution;
        for (int column = 0; column < rightHandSides; ++column) {
            const size_t offset = static_cast<size_t>(column)
                * static_cast<size_t>(n_);
            std::copy_n(rhs.begin() + static_cast<std::ptrdiff_t>(offset),
                        static_cast<size_t>(n_), columnRhs.begin());
            solveDenseCholesky(columnRhs, columnSolution);
            std::copy(columnSolution.begin(), columnSolution.end(),
...
public:
    ...
    void solve(const std::vector<double>& rhs, std::vector<double>& x)
    {
        solveInternal(rhs, 1, false, x);
    }

    void solveTranspose(const std::vector<double>& rhs,
                        std::vector<double>& x)
    {
        solveInternal(rhs, 1, true, x);
    }
    ...
    {
        if (rightHandSides <= 0
            || rhs.size() != static_cast<std::size_t>(n_)
                * static_cast<std::size_t>(rightHandSides)) {
            throw std::runtime_error(
                "General PARDISO multi-RHS dimensions are invalid.");
        }
        solveInternal(rhs, rightHandSides, transpose, x);
    }
...
private:
    void solveInternal(const std::vector<double>& rhs,
                       int rightHandSides,
                       bool transpose,
                       std::vector<double>& x)
    {
        if (rightHandSides <= 0
            || rhs.size() != static_cast<std::size_t>(n_)
                * static_cast<std::size_t>(rightHandSides)) {
            throw std::runtime_error(
                "General PARDISO solve dimensions are invalid.");
        }
#ifdef USE_MKL_PARDISO
        x.assign(rhs.size(), 0.0);
        MKL_INT phase = 33;
        MKL_INT nrhs = static_cast<MKL_INT>(rightHandSides);
        MKL_INT error = 0;
        const MKL_INT savedTranspose = iparm_[11];
        iparm_[11] = transpose ? 1 : 0;
        pardiso(pt_, &maxfct_, &mnum_, &mtype_, &phase, &n_,
                values_.data(), rowPtr_.data(), colInd_.data(), perm_.data(),
                &nrhs, iparm_, &msglvl_, const_cast<double*>(rhs.data()), x.data(), &error);
        iparm_[11] = savedTranspose;
        if (error != 0) {
    ...
    void factor(int n, const std::vector<MatrixEntry>& entries)
    {
        ...
        if (error != 0) {
            throw std::runtime_error(
                "General PARDISO symbolic analysis failed with error "
                + std::to_string(error));
        }
        phase = 22;
        error = 0;
        ...
        if (error != 0) {
            throw std::runtime_error(
                "General PARDISO numerical factorization failed with error "
                + std::to_string(error));
        }
        pardisoMemoryKb_ = std::max<MKL_INT>(0, iparm_[14] + iparm_[15] + iparm_[16]);
#else
    ...
    void moveFrom(GeneralSparseDirectSolver&& other) noexcept
    {
        ...
        perm_ = std::move(other.perm_);
        symbolicAnalysisSeconds_ = other.symbolicAnalysisSeconds_;
        numericalFactorizationSeconds_ =
            other.numericalFactorizationSeconds_;
        symbolicAnalysisCalls_ = other.symbolicAnalysisCalls_;
        numericalFactorizationCalls_ =
            other.numericalFactorizationCalls_;
#ifdef USE_MKL_PARDISO
        for (int i = 0; i < 64; ++i) {
            pt_[i] = other.pt_[i];
            other.pt_[i] = nullptr;
