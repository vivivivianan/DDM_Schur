// Build the algebraic domain decomposition used by both local ROM generation
// and the online interface solve. A P2 DOF is classified as interior only when
// all matrix support remains in one mesh domain; every shared/support-crossing
// DOF enters the globally ordered physical interface. This ordering is a cache
// and serialization invariant, so traversal is deterministic.
