...
struct ProgramOptions {
    ...
    bool runSchurDirectExact = false;
    bool runP2P1AuxPcg = false;
    bool runReducedSchur = false;
    bool runLocalRom = false;
    ...
    bool schurEnergyAdaptiveCoarse = false;
    int schurEnergyMaxModesPerDomain = 8;
    int schurEnergySubspaceIterations = 12;
    double schurEnergyEigenvalueThreshold = 0.1;
    bool schurGlobalSlowCoarse = false;
    int schurGlobalSlowModes = 6;
    int schurGlobalSlowSubspaceDimension = 100;
    bool schurProxyDiagnostics = false;
    bool schurProxyEnabled = true;
    bool schurProxyDisableCoarse = false;
    double schurProxyHighKThreshold = 20.0;
    bool schurProxyUseMaterialConnectivity = true;
    int schurProxyRing = 1;
    int schurProxyProbeColumns = 16;
    int schurProxyBlockSize = 64;
    bool schurProxyValidateBlockEquivalence = false;
    int schurLocalSolveThreads = 1;
    int schurLocalPardisoThreads = 1;
    bool schurProxyCacheEnabled = false;
    std::filesystem::path schurProxyCachePath;
    int schurInterfaceOperatorCoarseRank = 0;
    int schurInterfaceOperatorCoarseSweeps = 2;
    bool schurInterfaceOperatorCoarsePredictor = true;
    std::filesystem::path schurInterfaceOperatorCoarseCachePath;
    int schurMultiRhsCount = 0;
    bool schurDirectPatternOnly = false;
    bool schurDirectDryRun = false;
    int schurDirectBatchSize = 0;
    double schurDirectMemoryFraction = 0.50;
    double schurDirectFactorMemoryFraction = 0.70;
    bool schurDirectVerifyOperator = true;
    int schurDirectRandomChecks = 3;
    bool morGenerateModel = false;
    std::filesystem::path morLoadModelPath;
    std::filesystem::path morSaveModelPath;
    int morRank = 0;
    double morEnergyTolerance = 1.0e-10;
    double morSingularValueTolerance = 1.0e-12;
    int morTrainingCases = 60;
    int morValidationCases = 20;
    int morTestCases = 20;
    std::uint64_t morSeed = 20260721ULL;
    std::string morMode = "pure";
    std::string morSnapshotSolver = "auto";
    bool morExactInteriorRecovery = true;
    bool morCompareFom = false;
    std::vector<int> morRankSweep{5, 10, 20, 40, 60, 80, 100};
    std::string morInteriorMode = "pardiso";
    int morInteriorRank = 0;
    double morInteriorEnergyTolerance = 1.0e-8;
    double morInteriorSingularValueTolerance = 1.0e-12;
    std::vector<int> morInteriorRankSweep{5, 10, 20, 40, 60, 80, 100, 125};
    bool morDeploymentOnly = false;
    bool morPrecomputePowerResponse = false;
    std::string morStoragePrecision = "float64";
    bool morReportIoTime = false;
    bool morCompareInteriorModes = false;
    int morDeploymentRhsCount = 1;
    bool morParametricGenerate = false;
    bool morParametricDeploymentOnly = false;
    std::filesystem::path morParametricLoadPath;
    std::filesystem::path morParametricSavePath;
    std::string morMatrixParameter;
    int morParameterSubdomain = -1;
    int morParameterRegionId = -1;
    double morParameterMinimum = std::numeric_limits<double>::quiet_NaN();
    double morParameterMaximum = std::numeric_limits<double>::quiet_NaN();
    double morParameterReference = std::numeric_limits<double>::quiet_NaN();
    double morParameterValue = std::numeric_limits<double>::quiet_NaN();
    int morParameterTrainingCount = 5;
    int morParameterValidationCount = 4;
    int morParameterTestCount = 4;
    int morInterfaceRank = 0;
    int morLocalRank = 0;
    std::string morParametricMode = "pure";
    bool morAllowExtrapolation = false;
    bool morParametricAffineValidationOnly = false;
    std::vector<double> morOnlinePowersW;
    bool morTransientGenerate = false;
    bool morTransientDeploymentOnly = false;
    std::filesystem::path morTransientLoadPath;
    std::filesystem::path morTransientSavePath;
    std::string morTransientMethod = "block-arnoldi";
    int morArnoldiMoments = 2;
    double morArnoldiExpansionPoint = 0.0;
    double morArnoldiRankTolerance = 1.0e-10;
    double morArnoldiSecondMomentEnergy = 1.0;
    int morArnoldiSecondMomentMaximumColumns = 0;
    std::string morBasis = "standard";
    std::string morBasisOrthogonalization = "euclidean";
    std::string morProjection = "galerkin";
    double morBasisShift = 0.0;
    bool morBoundaryAwareBasis = false;
    bool morResidualEnrichment = false;
    int morResidualTopN = 4;
    int morResidualEnrichmentRounds = 1;
    int morResidualPilotSteps = 200;
    std::string morTransientMass = "consistent";
    double morTransientDt = std::numeric_limits<double>::quiet_NaN();
    double morTransientEndTime = std::numeric_limits<double>::quiet_NaN();
    std::string morTransientIntegrator = "backward-euler";
    std::filesystem::path morTransientInputPath;
    std::string morTransientWaveform = "multi_step";
    std::string morTransientOutput = "max-temperature";
    bool morTransientCompareFom = true;
    bool morTransientCompareFomSummaryOnly = false;
    bool morTransientFomOnly = false;
    ...
    std::vector<int> transientLocalRomRanks{10, 20, 40, 80};
    double transientLocalRomSvdTolerance = 1.0e-12;
    int transientLocalRomMaxSteps = 0;
    bool localMorGenerate = false;
    std::filesystem::path localMorLoadPath;
    std::filesystem::path localMorSavePath;
    std::string localMorMethod = "pod";
    std::string localMorMode = "pure";
    std::string localInterfaceMode = "full";
    int localMorRank = 0;
    std::vector<int> localMorRankPerSubdomain;
    std::filesystem::path localMorRankFile;
    double localMorEnergyTolerance = 1.0e-10;
    double localMorSingularValueTolerance = 1.0e-12;
    int localMorTrainingCases = 12;
    int localMorValidationCases = 8;
    int localMorTestCases = 8;
    bool localMorCompareFom = true;
    bool localMorReuseIdenticalSubdomains = false;
    int localMorMatrixFreeInterfaceThreshold = 20000;
...
static bool parseSchurCoarseOption(ProgramOptions& options,
                                   ...
                                   char* argv[])
{
    ...
    } else if (arg == "--schur-energy-adaptive-coarse") {
        options.schurEnergyAdaptiveCoarse = true;
    } else if (arg == "--no-schur-energy-adaptive-coarse") {
        options.schurEnergyAdaptiveCoarse = false;
    } else if (arg == "--schur-energy-max-modes-per-domain" && i + 1 < argc) {
        options.schurEnergyMaxModesPerDomain = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-energy-max-modes-per-domain=", 0) == 0) {
        options.schurEnergyMaxModesPerDomain = std::stoi(
            arg.substr(std::string("--schur-energy-max-modes-per-domain=").size()));
    } else if (arg == "--schur-energy-subspace-iterations" && i + 1 < argc) {
        options.schurEnergySubspaceIterations = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-energy-subspace-iterations=", 0) == 0) {
        options.schurEnergySubspaceIterations = std::stoi(
            arg.substr(std::string("--schur-energy-subspace-iterations=").size()));
    } else if (arg == "--schur-energy-eigenvalue-threshold" && i + 1 < argc) {
        options.schurEnergyEigenvalueThreshold = std::stod(argv[++i]);
    } else if (arg.rfind("--schur-energy-eigenvalue-threshold=", 0) == 0) {
        options.schurEnergyEigenvalueThreshold = std::stod(
            arg.substr(std::string("--schur-energy-eigenvalue-threshold=").size()));
    } else if (arg == "--schur-global-slow-coarse") {
        options.schurGlobalSlowCoarse = true;
    } else if (arg == "--no-schur-global-slow-coarse") {
        options.schurGlobalSlowCoarse = false;
    } else if (arg == "--schur-global-slow-modes" && i + 1 < argc) {
        options.schurGlobalSlowModes = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-global-slow-modes=", 0) == 0) {
        options.schurGlobalSlowModes = std::stoi(
            arg.substr(std::string("--schur-global-slow-modes=").size()));
    } else if ((arg == "--schur-global-slow-subspace-dimension"
                || arg == "--schur-global-slow-arnoldi-dimension")
               && i + 1 < argc) {
        options.schurGlobalSlowSubspaceDimension = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-global-slow-subspace-dimension=", 0) == 0) {
        options.schurGlobalSlowSubspaceDimension = std::stoi(
            arg.substr(std::string("--schur-global-slow-subspace-dimension=").size()));
    } else if (arg.rfind("--schur-global-slow-arnoldi-dimension=", 0) == 0) {
        options.schurGlobalSlowSubspaceDimension = std::stoi(
            arg.substr(std::string("--schur-global-slow-arnoldi-dimension=").size()));
    } else if (arg == "--schur-proxy") {
        options.schurProxyEnabled = true;
    } else if (arg == "--no-schur-proxy") {
        options.schurProxyEnabled = false;
    } else if (arg == "--schur-proxy-only") {
        options.schurProxyEnabled = true;
        options.schurProxyDisableCoarse = true;
    } else if (arg == "--schur-proxy-diagnostics") {
        options.schurProxyDiagnostics = true;
    } else if (arg == "--no-schur-proxy-diagnostics") {
        options.schurProxyDiagnostics = false;
    } else if (arg == "--schur-proxy-use-material-connectivity") {
        options.schurProxyUseMaterialConnectivity = true;
    } else if (arg == "--no-schur-proxy-use-material-connectivity") {
        options.schurProxyUseMaterialConnectivity = false;
    } else if (arg == "--schur-proxy-high-k-threshold" && i + 1 < argc) {
        options.schurProxyHighKThreshold = std::stod(argv[++i]);
    } else if (arg.rfind("--schur-proxy-high-k-threshold=", 0) == 0) {
        options.schurProxyHighKThreshold = std::stod(
            arg.substr(std::string("--schur-proxy-high-k-threshold=").size()));
    } else if (arg == "--schur-proxy-ring" && i + 1 < argc) {
        options.schurProxyRing = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-proxy-ring=", 0) == 0) {
        options.schurProxyRing = std::stoi(
            arg.substr(std::string("--schur-proxy-ring=").size()));
    } else if (arg == "--schur-proxy-probe-columns" && i + 1 < argc) {
        options.schurProxyProbeColumns = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-proxy-probe-columns=", 0) == 0) {
        options.schurProxyProbeColumns = std::stoi(
            arg.substr(std::string("--schur-proxy-probe-columns=").size()));
    } else if (arg == "--schur-proxy-block-size" && i + 1 < argc) {
        options.schurProxyBlockSize = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-proxy-block-size=", 0) == 0) {
        options.schurProxyBlockSize = std::stoi(
            arg.substr(std::string("--schur-proxy-block-size=").size()));
    } else if (arg == "--schur-proxy-validate-block-equivalence") {
        options.schurProxyValidateBlockEquivalence = true;
    } else if (arg == "--no-schur-proxy-validate-block-equivalence") {
        options.schurProxyValidateBlockEquivalence = false;
    } else if (arg == "--schur-local-solve-threads" && i + 1 < argc) {
        options.schurLocalSolveThreads = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-local-solve-threads=", 0) == 0) {
        options.schurLocalSolveThreads = std::stoi(
            arg.substr(std::string("--schur-local-solve-threads=").size()));
    } else if (arg == "--schur-local-pardiso-threads" && i + 1 < argc) {
        options.schurLocalPardisoThreads = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-local-pardiso-threads=", 0) == 0) {
        options.schurLocalPardisoThreads = std::stoi(
            arg.substr(std::string("--schur-local-pardiso-threads=").size()));
    } else if (arg == "--schur-proxy-cache" && i + 1 < argc) {
        options.schurProxyCacheEnabled = true;
        options.schurProxyCachePath = argv[++i];
    } else if (arg.rfind("--schur-proxy-cache=", 0) == 0) {
        options.schurProxyCacheEnabled = true;
        options.schurProxyCachePath =
            arg.substr(std::string("--schur-proxy-cache=").size());
    } else if (arg == "--no-schur-proxy-cache") {
        options.schurProxyCacheEnabled = false;
        options.schurProxyCachePath.clear();
    } else if (arg == "--schur-interface-operator-coarse-rank" && i + 1 < argc) {
        options.schurInterfaceOperatorCoarseRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-interface-operator-coarse-rank=", 0) == 0) {
        options.schurInterfaceOperatorCoarseRank = std::stoi(
            arg.substr(std::string("--schur-interface-operator-coarse-rank=").size()));
    } else if (arg == "--schur-interface-operator-coarse-sweeps" && i + 1 < argc) {
        options.schurInterfaceOperatorCoarseSweeps = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-interface-operator-coarse-sweeps=", 0) == 0) {
        options.schurInterfaceOperatorCoarseSweeps = std::stoi(
            arg.substr(std::string("--schur-interface-operator-coarse-sweeps=").size()));
    } else if (arg == "--schur-interface-operator-coarse-cache" && i + 1 < argc) {
        options.schurInterfaceOperatorCoarseCachePath = argv[++i];
    } else if (arg.rfind("--schur-interface-operator-coarse-cache=", 0) == 0) {
        options.schurInterfaceOperatorCoarseCachePath =
            arg.substr(std::string("--schur-interface-operator-coarse-cache=").size());
    } else if (arg == "--no-schur-interface-operator-coarse-predictor") {
        options.schurInterfaceOperatorCoarsePredictor = false;
    } else if (arg == "--schur-interface-operator-coarse-predictor") {
        options.schurInterfaceOperatorCoarsePredictor = true;
    } else if (arg == "--schur-multi-rhs-count" && i + 1 < argc) {
        options.schurMultiRhsCount = std::stoi(argv[++i]);
    } else if (arg.rfind("--schur-multi-rhs-count=", 0) == 0) {
        options.schurMultiRhsCount = std::stoi(
            arg.substr(std::string("--schur-multi-rhs-count=").size()));
    } else {
        return false;
    }
    return true;
...
static bool parseTransientLocalRomOption(ProgramOptions& options,
                                         ...
                                         char* argv[])
{
    if (arg == "--mor-interface-initial-guess" && i + 1 < argc) {
        options.morInterfaceInitialGuess = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-interface-initial-guess=", 0) == 0) {
        options.morInterfaceInitialGuess = lowerString(
            arg.substr(std::string("--mor-interface-initial-guess=").size()));
    } else if (arg == "--mor-interface-krylov" && i + 1 < argc) {
        options.morInterfaceKrylov = lowerString(argv[++i]);
        ...
        options.morAdaptiveInterfaceTolerance = std::stod(arg.substr(
            std::string("--mor-adaptive-interface-tolerance=").size()));
    } else if (arg == "--mor-transient-production") {
        options.morTransientCompareFom = false;
    } else if (arg == "--transient-local-rom-test") {
        options.transientLocalRomTest = true;
    } else if (arg == "--no-transient-local-rom-test") {
        options.transientLocalRomTest = false;
    } else if (arg == "--transient-local-rom-ranks" && i + 1 < argc) {
        options.transientLocalRomRanks = parseIntList(argv[++i]);
    } else if (arg.rfind("--transient-local-rom-ranks=", 0) == 0) {
        options.transientLocalRomRanks = parseIntList(
            arg.substr(std::string("--transient-local-rom-ranks=").size()));
    } else if (arg == "--transient-local-rom-svd-tolerance" && i + 1 < argc) {
        options.transientLocalRomSvdTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--transient-local-rom-svd-tolerance=", 0) == 0) {
        options.transientLocalRomSvdTolerance = std::stod(
            arg.substr(std::string("--transient-local-rom-svd-tolerance=").size()));
    } else if (arg == "--transient-local-rom-max-steps" && i + 1 < argc) {
        options.transientLocalRomMaxSteps = std::stoi(argv[++i]);
    } else if (arg.rfind("--transient-local-rom-max-steps=", 0) == 0) {
        options.transientLocalRomMaxSteps = std::stoi(
            arg.substr(std::string("--transient-local-rom-max-steps=").size()));
    } else {
        return false;
    }
    return true;
...
static bool parseHistoryCompressionOption(ProgramOptions& options,
                                          const std::string& arg,
                                          int& i,
                                          int argc,
                                          char* argv[])
{
    if (arg == "--history-compression-method" && i + 1 < argc) {
        options.historyCompressionMethod = lowerString(argv[++i]);
    } else if (arg.rfind("--history-compression-method=", 0) == 0) {
        options.historyCompressionMethod = lowerString(
            arg.substr(std::string("--history-compression-method=").size()));
    } else if (arg == "--history-compression-rank" && i + 1 < argc) {
        options.historyCompressionRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--history-compression-rank=", 0) == 0) {
        options.historyCompressionRank = std::stoi(
            arg.substr(std::string("--history-compression-rank=").size()));
    } else if (arg == "--history-compression-tolerance" && i + 1 < argc) {
        options.historyCompressionTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--history-compression-tolerance=", 0) == 0) {
        options.historyCompressionTolerance = std::stod(
            arg.substr(std::string(
                "--history-compression-tolerance=").size()));
    } else if (arg == "--history-compression-max-interface-pilot") {
        options.historyCompressionMaximumInterfacePilot = true;
    } else if (arg == "--milestone8-production-basis-only") {
        options.milestone8ProductionBasisOnly = true;
    } else if (arg == "--milestone8-adaptive-production") {
        options.milestone8AdaptiveProduction = true;
    } else if (arg == "--adaptive-port-local-pilot") {
        options.adaptivePortLocalPilot = true;
    } else if (arg == "--adaptive-port-interface-id"
               && i + 1 < argc) {
        options.adaptivePortInterfaceIds = {
            std::stoi(argv[++i])};
    } else if (arg.rfind("--adaptive-port-interface-id=", 0) == 0) {
        options.adaptivePortInterfaceIds = {std::stoi(
            arg.substr(std::string(
                "--adaptive-port-interface-id=").size()))};
    } else if (arg == "--adaptive-port-interface-ids"
               && i + 1 < argc) {
        options.adaptivePortInterfaceIds =
            parseIntList(argv[++i]);
    } else if (arg.rfind("--adaptive-port-interface-ids=", 0) == 0) {
        options.adaptivePortInterfaceIds = parseIntList(
            arg.substr(std::string(
                "--adaptive-port-interface-ids=").size()));
    } else if (arg == "--global-interface-coarse-prototype") {
        options.globalInterfaceCoarsePrototype = true;
    } else if (arg
               == "--global-interface-coarse-explicit-reference") {
        options.globalInterfaceCoarseExplicitReference = true;
    } else if (arg == "--global-interface-coarse-inverse-mode"
               && i + 1 < argc) {
        options.globalInterfaceCoarseInverseMode =
            lowerString(argv[++i]);
    } else if (arg.rfind(
            "--global-interface-coarse-inverse-mode=", 0) == 0) {
        options.globalInterfaceCoarseInverseMode =
            lowerString(arg.substr(std::string(
                "--global-interface-coarse-inverse-mode=").size()));
    } else if (arg == "--global-interface-coarse-rank"
               && i + 1 < argc) {
        options.globalInterfaceCoarseRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--global-interface-coarse-rank=", 0) == 0) {
        options.globalInterfaceCoarseRank = std::stoi(
            arg.substr(std::string(
                "--global-interface-coarse-rank=").size()));
    } else if (arg == "--global-interface-coarse-candidate-dimension"
               && i + 1 < argc) {
        options.globalInterfaceCoarseCandidateDimension =
            std::stoi(argv[++i]);
    } else if (arg.rfind(
            "--global-interface-coarse-candidate-dimension=", 0) == 0) {
        options.globalInterfaceCoarseCandidateDimension =
            std::stoi(arg.substr(std::string(
                "--global-interface-coarse-candidate-dimension=").size()));
    } else if (arg == "--global-interface-coarse-max-iters"
               && i + 1 < argc) {
        options.globalInterfaceCoarseMaximumIterations =
            std::stoi(argv[++i]);
    } else if (arg.rfind(
            "--global-interface-coarse-max-iters=", 0) == 0) {
        options.globalInterfaceCoarseMaximumIterations =
            std::stoi(arg.substr(std::string(
                "--global-interface-coarse-max-iters=").size()));
    } else if (arg == "--global-interface-coarse-inner-max-iters"
               && i + 1 < argc) {
        options.globalInterfaceCoarseInnerMaximumIterations =
            std::stoi(argv[++i]);
    } else if (arg.rfind(
            "--global-interface-coarse-inner-max-iters=", 0) == 0) {
        options.globalInterfaceCoarseInnerMaximumIterations =
            std::stoi(arg.substr(std::string(
                "--global-interface-coarse-inner-max-iters=").size()));
    } else if (arg == "--global-interface-coarse-krylov-sweeps"
               && i + 1 < argc) {
        options.globalInterfaceCoarseKrylovSweeps =
            std::stoi(argv[++i]);
    } else if (arg.rfind(
            "--global-interface-coarse-krylov-sweeps=", 0) == 0) {
        options.globalInterfaceCoarseKrylovSweeps =
            std::stoi(arg.substr(std::string(
                "--global-interface-coarse-krylov-sweeps=").size()));
    } else if (arg == "--global-interface-coarse-tol"
               && i + 1 < argc) {
        options.globalInterfaceCoarseTolerance =
            std::stod(argv[++i]);
    } else if (arg.rfind("--global-interface-coarse-tol=", 0) == 0) {
        options.globalInterfaceCoarseTolerance = std::stod(
            arg.substr(std::string(
                "--global-interface-coarse-tol=").size()));
    } else if (arg == "--global-interface-coarse-inner-tol"
               && i + 1 < argc) {
        options.globalInterfaceCoarseInnerTolerance =
            std::stod(argv[++i]);
    } else if (arg.rfind(
            "--global-interface-coarse-inner-tol=", 0) == 0) {
        options.globalInterfaceCoarseInnerTolerance =
            std::stod(arg.substr(std::string(
                "--global-interface-coarse-inner-tol=").size()));
    } else if (arg == "--global-interface-coarse-interface-ids"
               && i + 1 < argc) {
        options.globalInterfaceCoarseInterfaceIds =
            parseIntList(argv[++i]);
    } else if (arg.rfind(
            "--global-interface-coarse-interface-ids=", 0) == 0) {
        options.globalInterfaceCoarseInterfaceIds =
            parseIntList(arg.substr(std::string(
                "--global-interface-coarse-interface-ids=").size()));
    } else if (arg == "--global-randomized-schur") {
        options.globalRandomizedSchur = true;
    } else if (arg == "--global-randomized-rank"
               && i + 1 < argc) {
        options.globalRandomizedRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--global-randomized-rank=", 0) == 0) {
        options.globalRandomizedRank = std::stoi(
            arg.substr(std::string(
                "--global-randomized-rank=").size()));
    } else if (arg == "--global-randomized-seed"
               && i + 1 < argc) {
        options.globalRandomizedSeed =
            static_cast<std::uint64_t>(std::stoull(argv[++i]));
    } else if (arg.rfind("--global-randomized-seed=", 0) == 0) {
        options.globalRandomizedSeed =
            static_cast<std::uint64_t>(std::stoull(
                arg.substr(std::string(
                    "--global-randomized-seed=").size())));
    } else if (arg == "--global-randomized-inner-max-iters"
               && i + 1 < argc) {
        options.globalRandomizedInnerMaximumIterations =
            std::stoi(argv[++i]);
    } else if (arg.rfind(
            "--global-randomized-inner-max-iters=", 0) == 0) {
        options.globalRandomizedInnerMaximumIterations =
            std::stoi(arg.substr(std::string(
                "--global-randomized-inner-max-iters=").size()));
    } else if (arg == "--global-randomized-inner-tol"
               && i + 1 < argc) {
        options.globalRandomizedInnerTolerance =
            std::stod(argv[++i]);
    } else if (arg.rfind(
            "--global-randomized-inner-tol=", 0) == 0) {
        options.globalRandomizedInnerTolerance =
            std::stod(arg.substr(std::string(
                "--global-randomized-inner-tol=").size()));
    } else if (arg == "--global-randomized-composition"
               && i + 1 < argc) {
        options.globalRandomizedComposition =
            lowerString(argv[++i]);
    } else if (arg.rfind(
            "--global-randomized-composition=", 0) == 0) {
        options.globalRandomizedComposition =
            lowerString(arg.substr(std::string(
                "--global-randomized-composition=").size()));
    } else if (arg == "--milestone8-projection-diagnosis") {
        options.projectionDiagnosis = true;
    } else if (arg == "--milestone8-flux-operator-audit") {
        options.fluxOperatorAudit = true;
        options.projectionDiagnosis = true;
    } else if (arg == "--flux-aware-port") {
        options.fluxAwarePort = true;
    } else if (arg == "--flux-aware-flux-type"
               && i + 1 < argc) {
        options.fluxAwarePort = true;
        options.fluxAwareFluxType = lowerString(argv[++i]);
    } else if (arg.rfind(
            "--flux-aware-flux-type=", 0) == 0) {
        options.fluxAwarePort = true;
        options.fluxAwareFluxType = lowerString(arg.substr(
            std::string("--flux-aware-flux-type=").size()));
    } else if (arg == "--projection-interface-ids"
               && i + 1 < argc) {
        options.projectionInterfaceIds =
            parseIntList(argv[++i]);
    } else if (arg.rfind(
            "--projection-interface-ids=", 0) == 0) {
        options.projectionInterfaceIds =
            parseIntList(arg.substr(std::string(
                "--projection-interface-ids=").size()));
    } else {
        return false;
    }
    return true;
...
static bool parseLocalMorOption(ProgramOptions& options,
                                const std::string& arg,
                                int& i,
                                int argc,
                                char* argv[])
{
    if (arg == "--local-mor-generate") {
        options.localMorGenerate = true;
        options.runLocalRom = true;
    } else if (arg == "--local-mor-load" && i + 1 < argc) {
        options.localMorLoadPath = argv[++i];
        options.runLocalRom = true;
    } else if (arg.rfind("--local-mor-load=", 0) == 0) {
        options.localMorLoadPath = arg.substr(std::string("--local-mor-load=").size());
        options.runLocalRom = true;
    } else if (arg == "--local-mor-save" && i + 1 < argc) {
        options.localMorSavePath = argv[++i];
    } else if (arg.rfind("--local-mor-save=", 0) == 0) {
        options.localMorSavePath = arg.substr(std::string("--local-mor-save=").size());
    } else if (arg == "--local-mor-method" && i + 1 < argc) {
        options.localMorMethod = lowerString(argv[++i]);
    } else if (arg.rfind("--local-mor-method=", 0) == 0) {
        options.localMorMethod = lowerString(arg.substr(std::string("--local-mor-method=").size()));
    } else if ((arg == "--local-mor-mode" || arg == "--local-rom-mode")
               && i + 1 < argc) {
        options.localMorMode = lowerString(argv[++i]);
    } else if (arg.rfind("--local-mor-mode=", 0) == 0) {
        options.localMorMode = lowerString(arg.substr(std::string("--local-mor-mode=").size()));
    } else if (arg.rfind("--local-rom-mode=", 0) == 0) {
        options.localMorMode = lowerString(arg.substr(std::string("--local-rom-mode=").size()));
    } else if (arg == "--local-interface-mode" && i + 1 < argc) {
        options.localInterfaceMode = lowerString(argv[++i]);
    } else if (arg.rfind("--local-interface-mode=", 0) == 0) {
        options.localInterfaceMode = lowerString(arg.substr(std::string("--local-interface-mode=").size()));
    } else if (arg == "--local-mor-rank" && i + 1 < argc) {
        options.localMorRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--local-mor-rank=", 0) == 0) {
        options.localMorRank = std::stoi(arg.substr(std::string("--local-mor-rank=").size()));
    } else if (arg == "--local-mor-rank-per-subdomain" && i + 1 < argc) {
        options.localMorRankPerSubdomain = parseIntList(argv[++i]);
    } else if (arg.rfind("--local-mor-rank-per-subdomain=", 0) == 0) {
        options.localMorRankPerSubdomain = parseIntList(
            arg.substr(std::string("--local-mor-rank-per-subdomain=").size()));
    } else if (arg == "--local-mor-rank-file" && i + 1 < argc) {
        options.localMorRankFile = argv[++i];
    } else if (arg.rfind("--local-mor-rank-file=", 0) == 0) {
        options.localMorRankFile = arg.substr(std::string("--local-mor-rank-file=").size());
    } else if (arg == "--local-mor-energy-tolerance" && i + 1 < argc) {
        options.localMorEnergyTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--local-mor-energy-tolerance=", 0) == 0) {
        options.localMorEnergyTolerance = std::stod(
            arg.substr(std::string("--local-mor-energy-tolerance=").size()));
    } else if (arg == "--local-mor-singular-value-tolerance" && i + 1 < argc) {
        options.localMorSingularValueTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--local-mor-singular-value-tolerance=", 0) == 0) {
        options.localMorSingularValueTolerance = std::stod(
            arg.substr(std::string("--local-mor-singular-value-tolerance=").size()));
    } else if (arg == "--local-mor-training-cases" && i + 1 < argc) {
        options.localMorTrainingCases = std::stoi(argv[++i]);
    } else if (arg.rfind("--local-mor-training-cases=", 0) == 0) {
        options.localMorTrainingCases = std::stoi(
            arg.substr(std::string("--local-mor-training-cases=").size()));
    } else if (arg == "--local-mor-validation-cases" && i + 1 < argc) {
        options.localMorValidationCases = std::stoi(argv[++i]);
    } else if (arg.rfind("--local-mor-validation-cases=", 0) == 0) {
        options.localMorValidationCases = std::stoi(
            arg.substr(std::string("--local-mor-validation-cases=").size()));
    } else if (arg == "--local-mor-test-cases" && i + 1 < argc) {
        options.localMorTestCases = std::stoi(argv[++i]);
    } else if (arg.rfind("--local-mor-test-cases=", 0) == 0) {
        options.localMorTestCases = std::stoi(
            arg.substr(std::string("--local-mor-test-cases=").size()));
    } else if (arg == "--local-mor-compare-fom") {
        options.localMorCompareFom = true;
    } else if (arg == "--no-local-mor-compare-fom") {
        options.localMorCompareFom = false;
    } else if (arg == "--local-rom-reuse-identical-subdomains") {
        options.localMorReuseIdenticalSubdomains = true;
    } else if (arg == "--no-local-rom-reuse-identical-subdomains") {
        options.localMorReuseIdenticalSubdomains = false;
    } else if (arg == "--local-port-rank" && i + 1 < argc) {
        options.localPortRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--local-port-rank=", 0) == 0) {
        options.localPortRank = std::stoi(
            arg.substr(std::string("--local-port-rank=").size()));
    } else if (arg == "--port-basis-method" && i + 1 < argc) {
        options.portBasisMethod = lowerString(argv[++i]);
    } else if (arg.rfind("--port-basis-method=", 0) == 0) {
        options.portBasisMethod = lowerString(
            arg.substr(std::string("--port-basis-method=").size()));
    } else if (arg == "--optimal-port-rank" && i + 1 < argc) {
        options.optimalPortRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--optimal-port-rank=", 0) == 0) {
        options.optimalPortRank = std::stoi(
            arg.substr(std::string("--optimal-port-rank=").size()));
    } else if (arg == "--optimal-port-rank-mode" && i + 1 < argc) {
        options.optimalPortRankMode = lowerString(argv[++i]);
    } else if (arg.rfind("--optimal-port-rank-mode=", 0) == 0) {
        options.optimalPortRankMode = lowerString(
            arg.substr(std::string("--optimal-port-rank-mode=").size()));
    } else if (arg == "--optimal-port-rank-file" && i + 1 < argc) {
        options.optimalPortRankFile = argv[++i];
    } else if (arg.rfind("--optimal-port-rank-file=", 0) == 0) {
        options.optimalPortRankFile =
            arg.substr(std::string("--optimal-port-rank-file=").size());
    } else if (arg == "--optimal-port-eigenvalue-tol" && i + 1 < argc) {
        options.optimalPortEigenvalueTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--optimal-port-eigenvalue-tol=", 0) == 0) {
        options.optimalPortEigenvalueTolerance = std::stod(
            arg.substr(std::string("--optimal-port-eigenvalue-tol=").size()));
    } else if (arg == "--optimal-port-min-rank" && i + 1 < argc) {
        options.optimalPortMinimumRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--optimal-port-min-rank=", 0) == 0) {
        options.optimalPortMinimumRank = std::stoi(
            arg.substr(std::string("--optimal-port-min-rank=").size()));
    } else if (arg == "--optimal-port-max-rank" && i + 1 < argc) {
        options.optimalPortMaximumRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--optimal-port-max-rank=", 0) == 0) {
        options.optimalPortMaximumRank = std::stoi(
            arg.substr(std::string("--optimal-port-max-rank=").size()));
    } else if (arg == "--optimal-port-inner-product" && i + 1 < argc) {
        options.optimalPortInnerProduct = lowerString(argv[++i]);
    } else if (arg.rfind("--optimal-port-inner-product=", 0) == 0) {
        options.optimalPortInnerProduct = lowerString(
            arg.substr(std::string("--optimal-port-inner-product=").size()));
    } else if (arg == "--optimal-port-oversampling-layers" && i + 1 < argc) {
        options.optimalPortOversamplingLayers = std::stoi(argv[++i]);
    } else if (arg.rfind("--optimal-port-oversampling-layers=", 0) == 0) {
        options.optimalPortOversamplingLayers = std::stoi(
            arg.substr(std::string("--optimal-port-oversampling-layers=").size()));
    } else if (arg == "--optimal-port-inner-solver" && i + 1 < argc) {
        options.optimalPortInnerSolver = lowerString(argv[++i]);
    } else if (arg.rfind("--optimal-port-inner-solver=", 0) == 0) {
        options.optimalPortInnerSolver = lowerString(
            arg.substr(std::string("--optimal-port-inner-solver=").size()));
    } else if (arg == "--optimal-port-inner-tol" && i + 1 < argc) {
        options.optimalPortInnerTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--optimal-port-inner-tol=", 0) == 0) {
        options.optimalPortInnerTolerance = std::stod(
            arg.substr(std::string("--optimal-port-inner-tol=").size()));
    } else if (arg == "--optimal-port-inner-max-iters" && i + 1 < argc) {
        options.optimalPortInnerMaximumIterations = std::stoi(argv[++i]);
    } else if (arg.rfind("--optimal-port-inner-max-iters=", 0) == 0) {
        options.optimalPortInnerMaximumIterations = std::stoi(
            arg.substr(std::string("--optimal-port-inner-max-iters=").size()));
    } else if (arg == "--optimal-port-inner-refinement-max-iters"
               && i + 1 < argc) {
        options.optimalPortInnerRefinementMaximumIterations =
            std::stoi(argv[++i]);
    } else if (arg.rfind(
                   "--optimal-port-inner-refinement-max-iters=", 0)
               == 0) {
        options.optimalPortInnerRefinementMaximumIterations =
            std::stoi(arg.substr(std::string(
                "--optimal-port-inner-refinement-max-iters=").size()));
    } else if (arg == "--optimal-port-inner-refinement-tol"
               && i + 1 < argc) {
        options.optimalPortInnerRefinementTolerance =
            std::stod(argv[++i]);
    } else if (arg.rfind(
                   "--optimal-port-inner-refinement-tol=", 0)
               == 0) {
        options.optimalPortInnerRefinementTolerance =
            std::stod(arg.substr(std::string(
                "--optimal-port-inner-refinement-tol=").size()));
    } else if (arg == "--optimal-port-eigen-tol" && i + 1 < argc) {
        options.optimalPortEigenTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--optimal-port-eigen-tol=", 0) == 0) {
        options.optimalPortEigenTolerance = std::stod(
            arg.substr(std::string("--optimal-port-eigen-tol=").size()));
    } else if (arg == "--optimal-port-eigen-max-iters" && i + 1 < argc) {
        options.optimalPortEigenMaximumIterations = std::stoi(argv[++i]);
    } else if (arg.rfind("--optimal-port-eigen-max-iters=", 0) == 0) {
        options.optimalPortEigenMaximumIterations = std::stoi(
            arg.substr(std::string("--optimal-port-eigen-max-iters=").size()));
    } else if (arg == "--optimal-port-ablation" && i + 1 < argc) {
        options.optimalPortAblation = lowerString(argv[++i]);
    } else if (arg.rfind("--optimal-port-ablation=", 0) == 0) {
        options.optimalPortAblation = lowerString(
            arg.substr(std::string("--optimal-port-ablation=").size()));
    } else if (arg == "--optimal-port-source-mode" && i + 1 < argc) {
        options.optimalPortSourceMode = lowerString(argv[++i]);
    } else if (arg.rfind("--optimal-port-source-mode=", 0) == 0) {
        options.optimalPortSourceMode = lowerString(
            arg.substr(std::string("--optimal-port-source-mode=").size()));
    } else if (arg == "--optimal-port-topology-audit") {
        options.optimalPortTopologyAudit = true;
    } else if (arg == "--optimal-port-basis-pilot") {
        options.optimalPortBasisPilot = true;
    } else if (arg == "--optimal-port-target-solver-comparison") {
        options.optimalPortTargetSolverComparison = true;
    } else if (arg == "--optimal-port-woodbury-pilot") {
        options.optimalPortWoodburyPilot = true;
    } else if (arg == "--optimal-port-refinement-validation") {
        options.optimalPortRefinementValidation = true;
    } else if (arg == "--optimal-port-representative-interface-pilot") {
        options.optimalPortRepresentativeInterfacePilot = true;
    } else if (arg
               == "--optimal-port-maximum-interface-refinement-pilot") {
        options.optimalPortMaximumInterfaceRefinementPilot = true;
    } else if (arg == "--optimal-port-all-interface-basis") {
        options.optimalPortAllInterfaceBasis = true;
    } else if (arg == "--optimal-port-topology-audit-csv"
               && i + 1 < argc) {
        options.optimalPortTopologyAuditCsv = argv[++i];
    } else if (arg.rfind("--optimal-port-topology-audit-csv=", 0) == 0) {
        options.optimalPortTopologyAuditCsv =
            arg.substr(std::string(
                "--optimal-port-topology-audit-csv=").size());
    } else if (arg == "--residual-krylov-max-rank"
               && i + 1 < argc) {
        options.residualKrylovMaximumRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--residual-krylov-max-rank=", 0) == 0) {
        options.residualKrylovMaximumRank = std::stoi(
            arg.substr(std::string(
                "--residual-krylov-max-rank=").size()));
    } else if (arg == "--residual-krylov-max-sweeps"
               && i + 1 < argc) {
        options.residualKrylovMaximumSweeps = std::stoi(argv[++i]);
    } else if (arg.rfind("--residual-krylov-max-sweeps=", 0) == 0) {
        options.residualKrylovMaximumSweeps = std::stoi(
            arg.substr(std::string(
                "--residual-krylov-max-sweeps=").size()));
    } else if (arg == "--residual-krylov-tol"
               && i + 1 < argc) {
        options.residualKrylovTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--residual-krylov-tol=", 0) == 0) {
        options.residualKrylovTolerance = std::stod(
            arg.substr(std::string(
                "--residual-krylov-tol=").size()));
    } else if (arg == "--residual-krylov-block-size"
               && i + 1 < argc) {
        options.residualKrylovBlockSize = std::stoi(argv[++i]);
    } else if (arg.rfind("--residual-krylov-block-size=", 0) == 0) {
        options.residualKrylovBlockSize = std::stoi(
            arg.substr(std::string(
                "--residual-krylov-block-size=").size()));
    } else if (arg == "--residual-krylov-probe-mode"
               && i + 1 < argc) {
        options.residualKrylovProbeMode =
            lowerString(argv[++i]);
    } else if (arg.rfind("--residual-krylov-probe-mode=", 0) == 0) {
        options.residualKrylovProbeMode = lowerString(
            arg.substr(std::string(
                "--residual-krylov-probe-mode=").size()));
    } else if (arg == "--residual-krylov-inner-solver"
               && i + 1 < argc) {
        options.residualKrylovInnerSolver =
            lowerString(argv[++i]);
    } else if (arg.rfind("--residual-krylov-inner-solver=", 0) == 0) {
        options.residualKrylovInnerSolver = lowerString(
            arg.substr(std::string(
                "--residual-krylov-inner-solver=").size()));
    } else if (arg == "--residual-krylov-representative-pilot") {
        options.residualKrylovRepresentativePilot = true;
    } else if (arg == "--residual-krylov-all-interface-basis") {
        options.residualKrylovAllInterfaceBasis = true;
    } else if (arg == "--randomized-port-rank" && i + 1 < argc) {
        options.randomizedPortRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--randomized-port-rank=", 0) == 0) {
        options.randomizedPortRank = std::stoi(
            arg.substr(std::string("--randomized-port-rank=").size()));
    } else if (arg == "--randomized-oversampling" && i + 1 < argc) {
        options.randomizedPortOversampling = std::stoi(argv[++i]);
    } else if (arg.rfind("--randomized-oversampling=", 0) == 0) {
        options.randomizedPortOversampling = std::stoi(
            arg.substr(std::string("--randomized-oversampling=").size()));
    } else if (arg == "--randomized-power-iterations"
               && i + 1 < argc) {
        options.randomizedPortPowerIterations = std::stoi(argv[++i]);
    } else if (arg.rfind("--randomized-power-iterations=", 0) == 0) {
        options.randomizedPortPowerIterations = std::stoi(
            arg.substr(std::string(
                "--randomized-power-iterations=").size()));
    } else if (arg == "--randomized-seed" && i + 1 < argc) {
        options.randomizedPortSeed =
            static_cast<std::uint64_t>(std::stoull(argv[++i]));
    } else if (arg.rfind("--randomized-seed=", 0) == 0) {
        options.randomizedPortSeed = static_cast<std::uint64_t>(
            std::stoull(arg.substr(
                std::string("--randomized-seed=").size())));
    } else if (arg == "--randomized-port-compare-optimal") {
        options.randomizedPortCompareOptimal = true;
    } else if (arg == "--randomized-port-representative-pilot") {
        options.randomizedPortRepresentativePilot = true;
    } else if (arg == "--local-port-energy-tolerance" && i + 1 < argc) {
        options.localPortEnergyTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--local-port-energy-tolerance=", 0) == 0) {
        options.localPortEnergyTolerance = std::stod(
            arg.substr(std::string("--local-port-energy-tolerance=").size()));
    } else if (arg == "--local-port-rank-file" && i + 1 < argc) {
        options.localPortRankFile = argv[++i];
    } else if (arg.rfind("--local-port-rank-file=", 0) == 0) {
        options.localPortRankFile =
            arg.substr(std::string("--local-port-rank-file=").size());
    } else if (arg == "--local-port-temperature-weight" && i + 1 < argc) {
        options.localPortTemperatureWeight = std::stod(argv[++i]);
    } else if (arg.rfind("--local-port-temperature-weight=", 0) == 0) {
        options.localPortTemperatureWeight = std::stod(
            arg.substr(std::string("--local-port-temperature-weight=").size()));
    } else if (arg == "--local-port-flux-weight" && i + 1 < argc) {
        options.localPortFluxWeight = std::stod(argv[++i]);
    } else if (arg.rfind("--local-port-flux-weight=", 0) == 0) {
        options.localPortFluxWeight = std::stod(
            arg.substr(std::string("--local-port-flux-weight=").size()));
    } else if (arg == "--local-port-residual-weight" && i + 1 < argc) {
        options.localPortResidualWeight = std::stod(argv[++i]);
    } else if (arg.rfind("--local-port-residual-weight=", 0) == 0) {
        options.localPortResidualWeight = std::stod(
            arg.substr(std::string("--local-port-residual-weight=").size()));
    } else if (arg == "--local-port-enrichment-rounds" && i + 1 < argc) {
        options.localPortEnrichmentRounds = std::stoi(argv[++i]);
    } else if (arg.rfind("--local-port-enrichment-rounds=", 0) == 0) {
        options.localPortEnrichmentRounds = std::stoi(
            arg.substr(std::string("--local-port-enrichment-rounds=").size()));
    } else if (arg == "--local-port-corrected") {
        options.localPortCorrected = true;
    } else if (arg == "--no-local-port-corrected") {
        options.localPortCorrected = false;
    } else if (arg == "--local-mor-matrix-free-threshold" && i + 1 < argc) {
        options.localMorMatrixFreeInterfaceThreshold = std::stoi(argv[++i]);
    } else if (arg.rfind("--local-mor-matrix-free-threshold=", 0) == 0) {
        options.localMorMatrixFreeInterfaceThreshold = std::stoi(
            arg.substr(std::string("--local-mor-matrix-free-threshold=").size()));
    } else {
        return false;
    }
    return true;
...
static bool parseMorOption(ProgramOptions& options,
                            ...
                            char* argv[])
{
    const auto parseArnoldiSecondMomentOption = [&]() -> bool {
        if (arg == "--mor-arnoldi-second-moment-energy" && i + 1 < argc) {
            options.morArnoldiSecondMomentEnergy = std::stod(argv[++i]);
            return true;
        }
        if (arg.rfind("--mor-arnoldi-second-moment-energy=", 0) == 0) {
            options.morArnoldiSecondMomentEnergy = std::stod(arg.substr(
                std::string("--mor-arnoldi-second-moment-energy=").size()));
            return true;
        }
        if (arg == "--mor-arnoldi-second-moment-max-columns" && i + 1 < argc) {
            options.morArnoldiSecondMomentMaximumColumns = std::stoi(argv[++i]);
            return true;
        }
        if (arg.rfind("--mor-arnoldi-second-moment-max-columns=", 0) == 0) {
            options.morArnoldiSecondMomentMaximumColumns = std::stoi(arg.substr(
                std::string("--mor-arnoldi-second-moment-max-columns=").size()));
            return true;
        }
        if (arg == "--mor-transient-compare-fom-summary-only") {
            options.morTransientCompareFom = true;
            options.morTransientCompareFomSummaryOnly = true;
            return true;
        }
        if (arg == "--mor-transient-fom-only") {
            options.morTransientFomOnly = true;
            return true;
        }
        if (arg == "--mor-native-reduced-history") {
            options.morNativeReducedHistory = true;
            return true;
        }
        if (arg == "--no-mor-native-reduced-history") {
            options.morNativeReducedHistory = false;
            return true;
        }
        return false;
    };
    if (parseArnoldiSecondMomentOption()) {
        return true;
    }
    if (arg == "--mor-full-residual-tolerance" && i + 1 < argc) {
        options.morFullResidualTolerance = std::stod(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-full-residual-tolerance=", 0) == 0) {
        options.morFullResidualTolerance = std::stod(
    ...
    if (arg == "--mor-skip-reduced-schur-validation") {
        options.morSkipReducedSchurValidation = true;
        return true;
    }
    if (arg == "--mor-local-transient-reuse-identical-subdomains") {
        options.morTransientReuseIdenticalSubdomains = true;
        return true;
    }
    if (arg == "--no-mor-local-transient-reuse-identical-subdomains") {
    ...
    if (arg == "--mor-basis" && i + 1 < argc) {
        options.morBasis = lowerString(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-basis=", 0) == 0) {
        options.morBasis = lowerString(
            arg.substr(std::string("--mor-basis=").size()));
        return true;
    }
    if (arg == "--mor-basis-orthogonalization" && i + 1 < argc) {
        options.morBasisOrthogonalization = lowerString(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-basis-orthogonalization=", 0) == 0) {
        options.morBasisOrthogonalization = lowerString(arg.substr(
            std::string("--mor-basis-orthogonalization=").size()));
        return true;
    }
    if (arg == "--mor-projection" && i + 1 < argc) {
        options.morProjection = lowerString(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-projection=", 0) == 0) {
        options.morProjection = lowerString(
            arg.substr(std::string("--mor-projection=").size()));
        return true;
    }
    if (arg == "--mor-basis-shift" && i + 1 < argc) {
        options.morBasisShift = std::stod(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-basis-shift=", 0) == 0) {
        options.morBasisShift = std::stod(
            arg.substr(std::string("--mor-basis-shift=").size()));
        return true;
    }
    if (arg == "--mor-boundary-aware-basis") {
        options.morBoundaryAwareBasis = true;
        return true;
    }
    if (arg == "--mor-residual-enrichment") {
        options.morResidualEnrichment = true;
        return true;
    }
    if (arg == "--mor-residual-top-n" && i + 1 < argc) {
        options.morResidualTopN = std::stoi(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-residual-top-n=", 0) == 0) {
        options.morResidualTopN = std::stoi(arg.substr(
            std::string("--mor-residual-top-n=").size()));
        return true;
    }
    if (arg == "--mor-residual-enrichment-rounds" && i + 1 < argc) {
        options.morResidualEnrichmentRounds = std::stoi(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-residual-enrichment-rounds=", 0) == 0) {
        options.morResidualEnrichmentRounds = std::stoi(arg.substr(
            std::string("--mor-residual-enrichment-rounds=").size()));
        return true;
    }
    if (arg == "--mor-residual-pilot-steps" && i + 1 < argc) {
        options.morResidualPilotSteps = std::stoi(argv[++i]);
        return true;
    }
    if (arg.rfind("--mor-residual-pilot-steps=", 0) == 0) {
        options.morResidualPilotSteps = std::stoi(arg.substr(
            std::string("--mor-residual-pilot-steps=").size()));
        return true;
    }
    if (parseHistoryCompressionOption(options, arg, i, argc, argv)) {
        return true;
    }
    if (parseLocalMorOption(options, arg, i, argc, argv)) {
        return true;
    }
    if (arg == "--mor-transient-generate") {
        options.morTransientGenerate = true;
    } else if (arg == "--mor-transient-deployment-only") {
        options.morTransientDeploymentOnly = true;
    } else if (arg == "--mor-transient-load" && i + 1 < argc) {
        options.morTransientLoadPath = argv[++i];
    } else if (arg.rfind("--mor-transient-load=", 0) == 0) {
        options.morTransientLoadPath = arg.substr(std::string("--mor-transient-load=").size());
    } else if (arg == "--mor-transient-save" && i + 1 < argc) {
        options.morTransientSavePath = argv[++i];
    } else if (arg.rfind("--mor-transient-save=", 0) == 0) {
        options.morTransientSavePath = arg.substr(std::string("--mor-transient-save=").size());
    } else if (arg == "--mor-transient-method" && i + 1 < argc) {
        options.morTransientMethod = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-transient-method=", 0) == 0) {
        options.morTransientMethod = lowerString(arg.substr(std::string("--mor-transient-method=").size()));
    } else if (arg == "--mor-arnoldi-moments" && i + 1 < argc) {
        options.morArnoldiMoments = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-arnoldi-moments=", 0) == 0) {
        options.morArnoldiMoments = std::stoi(arg.substr(std::string("--mor-arnoldi-moments=").size()));
    } else if (arg == "--mor-arnoldi-expansion-point" && i + 1 < argc) {
        options.morArnoldiExpansionPoint = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-arnoldi-expansion-point=", 0) == 0) {
        options.morArnoldiExpansionPoint = std::stod(arg.substr(std::string("--mor-arnoldi-expansion-point=").size()));
    } else if (arg == "--mor-arnoldi-rank-tolerance" && i + 1 < argc) {
        options.morArnoldiRankTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-arnoldi-rank-tolerance=", 0) == 0) {
        options.morArnoldiRankTolerance = std::stod(arg.substr(std::string("--mor-arnoldi-rank-tolerance=").size()));
    } else if (arg == "--mor-transient-mass" && i + 1 < argc) {
        options.morTransientMass = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-transient-mass=", 0) == 0) {
        options.morTransientMass = lowerString(arg.substr(std::string("--mor-transient-mass=").size()));
    } else if (arg == "--mor-transient-dt" && i + 1 < argc) {
        options.morTransientDt = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-transient-dt=", 0) == 0) {
        options.morTransientDt = std::stod(arg.substr(std::string("--mor-transient-dt=").size()));
    } else if (arg == "--mor-transient-t-end" && i + 1 < argc) {
        options.morTransientEndTime = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-transient-t-end=", 0) == 0) {
        options.morTransientEndTime = std::stod(arg.substr(std::string("--mor-transient-t-end=").size()));
    } else if (arg == "--mor-transient-integrator" && i + 1 < argc) {
        options.morTransientIntegrator = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-transient-integrator=", 0) == 0) {
        options.morTransientIntegrator = lowerString(arg.substr(std::string("--mor-transient-integrator=").size()));
    } else if (arg == "--mor-transient-input" && i + 1 < argc) {
        options.morTransientInputPath = argv[++i];
    } else if (arg.rfind("--mor-transient-input=", 0) == 0) {
        options.morTransientInputPath = arg.substr(std::string("--mor-transient-input=").size());
    } else if (arg == "--mor-transient-waveform" && i + 1 < argc) {
        options.morTransientWaveform = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-transient-waveform=", 0) == 0) {
        options.morTransientWaveform = lowerString(arg.substr(std::string("--mor-transient-waveform=").size()));
    } else if (arg == "--mor-transient-output" && i + 1 < argc) {
        options.morTransientOutput = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-transient-output=", 0) == 0) {
        options.morTransientOutput = lowerString(arg.substr(std::string("--mor-transient-output=").size()));
    } else if (arg == "--mor-transient-compare-fom") {
        options.morTransientCompareFom = true;
    } else if (arg == "--mor-transient-initial-temperature" && i + 1 < argc) {
        options.morTransientInitialTemperature = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-transient-initial-temperature=", 0) == 0) {
        options.morTransientInitialTemperature = std::stod(arg.substr(std::string("--mor-transient-initial-temperature=").size()));
    } else if (arg == "--mor-transient-initial-mode" && i + 1 < argc) {
        options.morTransientInitialMode = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-transient-initial-mode=", 0) == 0) {
        options.morTransientInitialMode = lowerString(
            arg.substr(std::string("--mor-transient-initial-mode=").size()));
    } else if (arg == "--mor-parametric-generate") {
        options.morParametricGenerate = true;
    } else if (arg == "--mor-parametric-deployment-only") {
        options.morParametricDeploymentOnly = true;
    } else if (arg == "--mor-parametric-load" && i + 1 < argc) {
        options.morParametricLoadPath = argv[++i];
    } else if (arg.rfind("--mor-parametric-load=", 0) == 0) {
        options.morParametricLoadPath = arg.substr(std::string("--mor-parametric-load=").size());
    } else if (arg == "--mor-parametric-save" && i + 1 < argc) {
        options.morParametricSavePath = argv[++i];
    } else if (arg.rfind("--mor-parametric-save=", 0) == 0) {
        options.morParametricSavePath = arg.substr(std::string("--mor-parametric-save=").size());
    } else if (arg == "--mor-matrix-parameter" && i + 1 < argc) {
        options.morMatrixParameter = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-matrix-parameter=", 0) == 0) {
        options.morMatrixParameter = lowerString(arg.substr(std::string("--mor-matrix-parameter=").size()));
    } else if (arg == "--mor-parameter-subdomain" && i + 1 < argc) {
        options.morParameterSubdomain = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-parameter-subdomain=", 0) == 0) {
        options.morParameterSubdomain = std::stoi(arg.substr(std::string("--mor-parameter-subdomain=").size()));
    } else if (arg == "--mor-parameter-region-id" && i + 1 < argc) {
        options.morParameterRegionId = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-parameter-region-id=", 0) == 0) {
        options.morParameterRegionId = std::stoi(arg.substr(std::string("--mor-parameter-region-id=").size()));
    } else if (arg == "--mor-parameter-min" && i + 1 < argc) {
        options.morParameterMinimum = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-parameter-min=", 0) == 0) {
        options.morParameterMinimum = std::stod(arg.substr(std::string("--mor-parameter-min=").size()));
    } else if (arg == "--mor-parameter-max" && i + 1 < argc) {
        options.morParameterMaximum = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-parameter-max=", 0) == 0) {
        options.morParameterMaximum = std::stod(arg.substr(std::string("--mor-parameter-max=").size()));
    } else if (arg == "--mor-parameter-reference" && i + 1 < argc) {
        options.morParameterReference = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-parameter-reference=", 0) == 0) {
        options.morParameterReference = std::stod(arg.substr(std::string("--mor-parameter-reference=").size()));
    } else if (arg == "--mor-parameter-value" && i + 1 < argc) {
        options.morParameterValue = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-parameter-value=", 0) == 0) {
        options.morParameterValue = std::stod(arg.substr(std::string("--mor-parameter-value=").size()));
    } else if (arg == "--mor-parameter-training-count" && i + 1 < argc) {
        options.morParameterTrainingCount = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-parameter-training-count=", 0) == 0) {
        options.morParameterTrainingCount = std::stoi(arg.substr(std::string("--mor-parameter-training-count=").size()));
    } else if (arg == "--mor-parameter-validation-count" && i + 1 < argc) {
        options.morParameterValidationCount = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-parameter-validation-count=", 0) == 0) {
        options.morParameterValidationCount = std::stoi(arg.substr(std::string("--mor-parameter-validation-count=").size()));
    } else if (arg == "--mor-parameter-test-count" && i + 1 < argc) {
        options.morParameterTestCount = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-parameter-test-count=", 0) == 0) {
        options.morParameterTestCount = std::stoi(arg.substr(std::string("--mor-parameter-test-count=").size()));
    } else if (arg == "--mor-interface-rank" && i + 1 < argc) {
        options.morInterfaceRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-interface-rank=", 0) == 0) {
        options.morInterfaceRank = std::stoi(arg.substr(std::string("--mor-interface-rank=").size()));
    } else if (arg == "--mor-local-rank" && i + 1 < argc) {
        options.morLocalRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-local-rank=", 0) == 0) {
        options.morLocalRank = std::stoi(arg.substr(std::string("--mor-local-rank=").size()));
    } else if (arg == "--mor-parametric-mode" && i + 1 < argc) {
        options.morParametricMode = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-parametric-mode=", 0) == 0) {
        options.morParametricMode = lowerString(arg.substr(std::string("--mor-parametric-mode=").size()));
    } else if (arg == "--mor-allow-extrapolation") {
        options.morAllowExtrapolation = true;
    } else if (arg == "--mor-parametric-affine-validation-only") {
        options.morParametricAffineValidationOnly = true;
    } else if (arg == "--mor-power-values" && i + 1 < argc) {
        options.morOnlinePowersW = parseDoubleList(argv[++i]);
    } else if (arg.rfind("--mor-power-values=", 0) == 0) {
        options.morOnlinePowersW = parseDoubleList(arg.substr(std::string("--mor-power-values=").size()));
    } else if (arg == "--mor-generate-model") {
        options.morGenerateModel = true;
    } else if (arg == "--mor-load-model" && i + 1 < argc) {
        options.morLoadModelPath = argv[++i];
    } else if (arg.rfind("--mor-load-model=", 0) == 0) {
        options.morLoadModelPath = arg.substr(std::string("--mor-load-model=").size());
    } else if (arg == "--mor-save-model" && i + 1 < argc) {
        options.morSaveModelPath = argv[++i];
    } else if (arg.rfind("--mor-save-model=", 0) == 0) {
        options.morSaveModelPath = arg.substr(std::string("--mor-save-model=").size());
    } else if (arg == "--mor-rank" && i + 1 < argc) {
        options.morRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-rank=", 0) == 0) {
        options.morRank = std::stoi(arg.substr(std::string("--mor-rank=").size()));
    } else if (arg == "--mor-energy-tolerance" && i + 1 < argc) {
        options.morEnergyTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-energy-tolerance=", 0) == 0) {
        options.morEnergyTolerance = std::stod(arg.substr(std::string("--mor-energy-tolerance=").size()));
    } else if (arg == "--mor-singular-value-tolerance" && i + 1 < argc) {
        options.morSingularValueTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-singular-value-tolerance=", 0) == 0) {
        options.morSingularValueTolerance = std::stod(arg.substr(std::string("--mor-singular-value-tolerance=").size()));
    } else if ((arg == "--mor-training-cases" || arg == "--mor-training-count")
               && i + 1 < argc) {
        options.morTrainingCases = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-training-cases=", 0) == 0) {
        options.morTrainingCases = std::stoi(arg.substr(std::string("--mor-training-cases=").size()));
    } else if (arg.rfind("--mor-training-count=", 0) == 0) {
        options.morTrainingCases = std::stoi(arg.substr(std::string("--mor-training-count=").size()));
    } else if ((arg == "--mor-validation-cases" || arg == "--mor-validation-count")
               && i + 1 < argc) {
        options.morValidationCases = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-validation-cases=", 0) == 0) {
        options.morValidationCases = std::stoi(arg.substr(std::string("--mor-validation-cases=").size()));
    } else if (arg.rfind("--mor-validation-count=", 0) == 0) {
        options.morValidationCases = std::stoi(arg.substr(std::string("--mor-validation-count=").size()));
    } else if ((arg == "--mor-test-cases" || arg == "--mor-test-count")
               && i + 1 < argc) {
        options.morTestCases = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-test-cases=", 0) == 0) {
        options.morTestCases = std::stoi(arg.substr(std::string("--mor-test-cases=").size()));
    } else if (arg.rfind("--mor-test-count=", 0) == 0) {
        options.morTestCases = std::stoi(arg.substr(std::string("--mor-test-count=").size()));
    } else if ((arg == "--mor-seed" || arg == "--mor-random-seed")
               && i + 1 < argc) {
        options.morSeed = std::stoull(argv[++i]);
    } else if (arg.rfind("--mor-seed=", 0) == 0) {
        options.morSeed = std::stoull(arg.substr(std::string("--mor-seed=").size()));
    } else if (arg.rfind("--mor-random-seed=", 0) == 0) {
        options.morSeed = std::stoull(arg.substr(std::string("--mor-random-seed=").size()));
    } else if ((arg == "--mor-mode" || arg == "--reduced-schur-mode")
               && i + 1 < argc) {
        options.morMode = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-mode=", 0) == 0) {
        options.morMode = lowerString(arg.substr(std::string("--mor-mode=").size()));
    } else if (arg.rfind("--reduced-schur-mode=", 0) == 0) {
        options.morMode = lowerString(arg.substr(std::string("--reduced-schur-mode=").size()));
    } else if (arg == "--mor-snapshot-solver" && i + 1 < argc) {
        options.morSnapshotSolver = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-snapshot-solver=", 0) == 0) {
        options.morSnapshotSolver = lowerString(
            arg.substr(std::string("--mor-snapshot-solver=").size()));
    } else if (arg == "--mor-exact-interior-recovery") {
        options.morExactInteriorRecovery = true;
    } else if (arg == "--no-mor-exact-interior-recovery") {
        options.morExactInteriorRecovery = false;
    } else if (arg == "--mor-compare-fom") {
        options.morCompareFom = true;
    } else if (arg == "--no-mor-compare-fom") {
        options.morCompareFom = false;
    } else if (arg == "--mor-rank-sweep" && i + 1 < argc) {
        options.morRankSweep = parseIntList(argv[++i]);
    } else if (arg.rfind("--mor-rank-sweep=", 0) == 0) {
        options.morRankSweep = parseIntList(arg.substr(std::string("--mor-rank-sweep=").size()));
    } else if (arg == "--mor-interior-mode" && i + 1 < argc) {
        options.morInteriorMode = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-interior-mode=", 0) == 0) {
        options.morInteriorMode = lowerString(
            arg.substr(std::string("--mor-interior-mode=").size()));
    } else if (arg == "--mor-interior-rank" && i + 1 < argc) {
        options.morInteriorRank = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-interior-rank=", 0) == 0) {
        options.morInteriorRank = std::stoi(
            arg.substr(std::string("--mor-interior-rank=").size()));
    } else if (arg == "--mor-interior-energy-tolerance" && i + 1 < argc) {
        options.morInteriorEnergyTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-interior-energy-tolerance=", 0) == 0) {
        options.morInteriorEnergyTolerance = std::stod(
            arg.substr(std::string("--mor-interior-energy-tolerance=").size()));
    } else if (arg == "--mor-interior-singular-value-tolerance" && i + 1 < argc) {
        options.morInteriorSingularValueTolerance = std::stod(argv[++i]);
    } else if (arg.rfind("--mor-interior-singular-value-tolerance=", 0) == 0) {
        options.morInteriorSingularValueTolerance = std::stod(
            arg.substr(std::string("--mor-interior-singular-value-tolerance=").size()));
    } else if (arg == "--mor-interior-rank-sweep" && i + 1 < argc) {
        options.morInteriorRankSweep = parseIntList(argv[++i]);
    } else if (arg.rfind("--mor-interior-rank-sweep=", 0) == 0) {
        options.morInteriorRankSweep = parseIntList(
            arg.substr(std::string("--mor-interior-rank-sweep=").size()));
    } else if (arg == "--mor-deployment-only") {
        options.morDeploymentOnly = true;
    } else if (arg == "--mor-precompute-power-response") {
        options.morPrecomputePowerResponse = true;
    } else if (arg == "--no-mor-precompute-power-response") {
        options.morPrecomputePowerResponse = false;
    } else if (arg == "--mor-storage-precision" && i + 1 < argc) {
        options.morStoragePrecision = lowerString(argv[++i]);
    } else if (arg.rfind("--mor-storage-precision=", 0) == 0) {
        options.morStoragePrecision = lowerString(
            arg.substr(std::string("--mor-storage-precision=").size()));
    } else if (arg == "--mor-report-io-time") {
        options.morReportIoTime = true;
    } else if (arg == "--mor-compare-interior-modes") {
        options.morCompareInteriorModes = true;
    } else if (arg == "--mor-deployment-rhs-count" && i + 1 < argc) {
        options.morDeploymentRhsCount = std::stoi(argv[++i]);
    } else if (arg.rfind("--mor-deployment-rhs-count=", 0) == 0) {
        options.morDeploymentRhsCount = std::stoi(
            arg.substr(std::string("--mor-deployment-rhs-count=").size()));
    } else {
        return false;
    }
    return true;
...
static bool parseExactSchurDirectOption(ProgramOptions& options,
                                        const std::string& arg,
                                        int& index,
                                        int argc,
                                        char* argv[])
{
    const auto valueAfter = [&](const std::string& prefix) {
        return arg.substr(prefix.size());
    };
    if (arg == "--schur-direct-pattern-only") {
        options.schurDirectPatternOnly = true;
    } else if (arg == "--schur-direct-dry-run") {
        options.schurDirectDryRun = true;
    } else if (arg == "--schur-direct-batch-size" && index + 1 < argc) {
        options.schurDirectBatchSize = std::stoi(argv[++index]);
    } else if (arg.rfind("--schur-direct-batch-size=", 0) == 0) {
        options.schurDirectBatchSize = std::stoi(valueAfter("--schur-direct-batch-size="));
    } else if (arg == "--schur-direct-memory-fraction" && index + 1 < argc) {
        options.schurDirectMemoryFraction = std::stod(argv[++index]);
    } else if (arg.rfind("--schur-direct-memory-fraction=", 0) == 0) {
        options.schurDirectMemoryFraction = std::stod(valueAfter("--schur-direct-memory-fraction="));
    } else if (arg == "--schur-direct-factor-memory-fraction" && index + 1 < argc) {
        options.schurDirectFactorMemoryFraction = std::stod(argv[++index]);
    } else if (arg.rfind("--schur-direct-factor-memory-fraction=", 0) == 0) {
        options.schurDirectFactorMemoryFraction = std::stod(valueAfter("--schur-direct-factor-memory-fraction="));
    } else if (arg == "--schur-direct-verify-operator" && index + 1 < argc) {
        options.schurDirectVerifyOperator = parseBoolValue(argv[++index]);
    } else if (arg.rfind("--schur-direct-verify-operator=", 0) == 0) {
        options.schurDirectVerifyOperator = parseBoolValue(valueAfter("--schur-direct-verify-operator="));
    } else if (arg == "--schur-direct-random-checks" && index + 1 < argc) {
        options.schurDirectRandomChecks = std::stoi(argv[++index]);
    } else if (arg.rfind("--schur-direct-random-checks=", 0) == 0) {
        options.schurDirectRandomChecks = std::stoi(valueAfter("--schur-direct-random-checks="));
    } else {
        return false;
    }
    return true;
...
static ProgramOptions parseProgramOptions(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (parseSchurCoarseOption(options, arg, i, argc, argv)) {
            continue;
        }
        if (parseTransientLocalRomOption(options, arg, i, argc, argv)) {
            continue;
        }
        if (parseExactSchurDirectOption(options, arg, i, argc, argv)) {
            continue;
        }
        if (parseMorOption(options, arg, i, argc, argv)) {
            continue;
        }
        if (arg == "--steady" || arg == "steady") {
            ...
            options.runSchurDirectExact = false;
            options.runP2P1AuxPcg = false;
            options.runReducedSchur = false;
            options.runLocalRom = false;
            options.transientLocalRomTest = false;
            ...
            for (const std::string& solver : splitCsv(argv[++i])) {
                ...
                options.runDdmSchur = options.runDdmSchur
                    ...
                    || name == "ddm-schur"
                    || name == "schur-fgmres";
                options.runSchurDirectExact = options.runSchurDirectExact
                    || name == "schur-direct-exact"
                    || name == "schur_direct_exact"
                    || name == "exact-schur-direct";
                options.runP2P1AuxPcg = options.runP2P1AuxPcg
                    || name == "p2-p1-aux-pcg";
                options.runReducedSchur = options.runReducedSchur
                    || name == "reduced-schur"
                    || name == "mor-schur"
                    || name == "schur-rom";
                options.runLocalRom = options.runLocalRom
                    || name == "local-rom"
                    || name == "local-rom-schur"
                    || name == "local-pod-schur";
                options.transientLocalRomTest = options.transientLocalRomTest
                    || name == "transient-local-rom-test"
                    || name == "transient_local_rom_test";
            ...
            std::cout << "Usage: SIPGHeatDDM3D.exe [--transient|--steady] [--config case_config.txt] [--solvers reduced-schur,schur,schur-direct-exact,direct,...]\n";
            ...
            std::cout << "  --solvers    comma-separated subset including reduced-schur, schur, schur-direct-exact, direct, and legacy benchmark solvers\n";
            ...
            std::cout << "  --[no-]schur-energy-adaptive-coarse  use local generalized Schur-energy eigenmodes\n";
            std::cout << "  --schur-energy-max-modes-per-domain value  maximum selected eigenmodes per subdomain (default 8)\n";
            std::cout << "  --schur-energy-subspace-iterations value  inverse subspace iterations (default 12)\n";
            std::cout << "  --schur-energy-eigenvalue-threshold value  select lambda <= threshold (default 0.1)\n";
            std::cout << "  --[no-]schur-global-slow-coarse  add matrix-free global Schur slow modes\n";
            std::cout << "  --schur-global-slow-modes value  selected global harmonic Ritz modes (default 6)\n";
            std::cout << "  --schur-global-slow-subspace-dimension value  harmonic Arnoldi dimension (default 100)\n";
            std::cout << "  --[no-]schur-proxy-diagnostics  run exact Schur locality diagnostics before proxy construction\n";
            std::cout << "  --[no-]schur-proxy  use the locality-approved sparse proxy as Level-1 preconditioner\n";
            std::cout << "  --schur-proxy-only  use proxy Level-1 without a coarse correction\n";
            std::cout << "  --schur-proxy-high-k-threshold value  conductivity threshold for physics graph links\n";
            std::cout << "  --[no-]schur-proxy-use-material-connectivity  add sparse high-k component graph links\n";
            std::cout << "  --schur-proxy-ring 1|2|3  requested proxy graph ring\n";
            std::cout << "  --schur-proxy-probe-columns value  stratified exact columns for locality diagnostics\n";
            std::cout << "  --schur-proxy-block-size value  multi-RHS probing block (default 64; use 1 for scalar reference)\n";
            std::cout << "  --schur-proxy-validate-block-equivalence  rebuild scalar proxy values and enforce a 1e-10 relative difference gate\n";
            std::cout << "  --schur-local-solve-threads value  OpenMP threads for independent subdomain solves (default 1)\n";
            std::cout << "  --schur-local-pardiso-threads value  MKL threads inside local and ring-1 proxy PARDISO instances (default 1)\n";
            std::cout << "  --schur-proxy-cache path  reuse a fingerprint-validated proxy graph, coloring, and matrix; factors are reused within one process\n";
            std::cout << "  --no-schur-proxy-cache  disable persistent proxy cache reuse\n";
            std::cout << "  --schur-interface-operator-coarse-rank value  add deterministic (I-P^-1 S) interface coarse modes (default 0)\n";
            std::cout << "  --schur-interface-operator-coarse-sweeps value  error-propagation depth for operator coarse modes (default 2)\n";
            std::cout << "  --schur-interface-operator-coarse-cache path  cache validated Q, S*Q, and coarse factor data\n";
            std::cout << "  --no-schur-interface-operator-coarse-predictor  keep operator modes in the preconditioner but skip the initial coarse prediction\n";
            std::cout << "  --schur-multi-rhs-count value  solve deterministic heat-source RHS ensemble with one Schur setup\n";
            std::cout << "  --mor-generate-model           generate source-channel snapshots and an exact reduced Schur model\n";
            std::cout << "  --mor-load-model path          load a saved model after strict mesh/matrix/interface/source fingerprint checks\n";
            std::cout << "  --mor-save-model path          save metadata, basis, reference state, S*V, and reduced operator\n";
            std::cout << "  --mor-rank value               selected POD rank; 0 uses the energy tolerance\n";
            std::cout << "  --mor-energy-tolerance value   maximum discarded snapshot energy (default 1e-10)\n";
            std::cout << "  --mor-singular-value-tolerance value  relative numerical-rank threshold (default 1e-12)\n";
            std::cout << "  --mor-training-cases value     deterministic training case count; all unit channels are always included\n";
            std::cout << "  --mor-validation-cases value   deterministic validation count (default 20)\n";
            std::cout << "  --mor-test-cases value         deterministic test count (default 20)\n";
            std::cout << "  --mor-seed value               fixed parameter-case seed\n";
            std::cout << "  --mor-mode pure|corrected      pure ROM or ROM-initialized exact FGMRES correction\n";
            std::cout << "  --mor-snapshot-solver auto|direct|schur  reusable FOM backend for offline snapshots/truth\n";
            std::cout << "  --[no-]mor-exact-interior-recovery  exact local-factor recovery (required in Stage 2A)\n";
            std::cout << "  --[no-]mor-compare-fom         run independent validation/test FOM truth solves\n";
            std::cout << "  --mor-rank-sweep csv           benchmark ranks (default 5,10,20,40,60,80,100)\n";
            std::cout << "  --mor-interior-mode pardiso|exact-response|compressed-rb  select exact factors or a fixed-A interior ROM\n";
            std::cout << "  --mor-interior-rank value      fixed local compressed rank; 0 uses energy tolerance\n";
            std::cout << "  --mor-interior-energy-tolerance value  maximum discarded local-response energy\n";
            std::cout << "  --mor-interior-singular-value-tolerance value  local numerical-rank threshold\n";
            std::cout << "  --mor-interior-rank-sweep csv  compressed local-rank validation sweep\n";
            std::cout << "  --mor-precompute-power-response  construct the direct fixed-A input-to-temperature map\n";
            std::cout << "  --mor-deployment-only          load only deployment response data; no config, FEM, Schur, FGMRES, or PARDISO\n";
            std::cout << "  --mor-deployment-rhs-count n   deterministic deployment timing batch (default 1)\n";
            std::cout << "  --mor-storage-precision float64|float32  float64 is the accuracy default\n";
            std::cout << "  --mor-report-io-time           report output I/O separately from ROM compute\n";
            std::cout << "  --mor-compare-interior-modes   compare PARDISO, exact-response, and compressed reconstruction\n";
            std::cout << "  --mor-parametric-generate      build a Stage 2B.1 local-RB / Reduced-Schur model for one matrix parameter\n";
            std::cout << "  --mor-parametric-load path     load a Stage 2B.1 model with strict mesh/operator/source fingerprints\n";
            std::cout << "  --mor-parametric-save path     save the self-contained Stage 2B.1 deployment model\n";
            std::cout << "  --mor-parametric-deployment-only  pure dense-ROM execution; no config, FEM, Schur, FGMRES, or PARDISO\n";
            std::cout << "  --mor-matrix-parameter convection-h|material-k  select the single active matrix parameter\n";
            std::cout << "  --mor-parameter-subdomain id   select the parameterized subdomain (-1 means wildcard where supported)\n";
            std::cout << "  --mor-parameter-region-id id   physical boundary tag or material domain/entity ID; never a coordinate box\n";
            std::cout << "  --mor-parameter-min/max/reference value  required physical training interval and reference value\n";
            std::cout << "  --mor-parameter-value value    online physical parameter value (default: model reference)\n";
            std::cout << "  --mor-parameter-training-count n  offline parameter samples, at least 3 (recommended 5 or 7)\n";
            std::cout << "  --mor-parameter-validation-count n  unseen midpoint validation samples\n";
            std::cout << "  --mor-parameter-test-count n   unseen fixed-seed random test samples\n";
            std::cout << "  --mor-interface-rank r         maximum Stage 2B.1 interface POD rank\n";
            std::cout << "  --mor-local-rank r             maximum POD rank for every subdomain interior\n";
            std::cout << "  --mor-parametric-mode pure|corrected  dense-only ROM or unchanged Stage 1 residual-gated correction\n";
            std::cout << "  --mor-power-values csv         online power in watts for every independent source channel\n";
            std::cout << "  --mor-allow-extrapolation      explicitly permit and flag pure-ROM values outside the training interval\n";
            std::cout << "  --mor-transient-generate      build a Stage 2C.1 physics-based Block Arnoldi model\n";
            std::cout << "  --mor-transient-load/save path  load or save a transient Block Arnoldi model\n";
            std::cout << "  --mor-transient-method block-arnoldi|local-block-arnoldi|local-port-block-arnoldi|local-interior-arnoldi-reduced-schur  transient MOR method\n";
            std::cout << "  --port-basis-method full-interface|port-pod|steklov-schur|mandatory-only|residual-krylov|optimal-transfer|randomized-transfer|hybrid-randomized  interface basis (default port-pod)\n";
            std::cout << "  --optimal-port-rank N             fixed total rank per physical interface\n";
            std::cout << "  --optimal-port-rank-mode fixed|match-m7|eigenvalue-tolerance\n";
            std::cout << "  --optimal-port-rank-file path     per-interface M7 rank CSV for match-m7\n";
            std::cout << "  --optimal-port-eigenvalue-tol x   transfer truncation criterion\n";
            std::cout << "  --optimal-port-min-rank N --optimal-port-max-rank N\n";
            std::cout << "  --optimal-port-inner-product trace-mass|penalty-weighted-mass\n";
            std::cout << "  --optimal-port-oversampling-layers 0|1|2\n";
            std::cout << "  --optimal-port-inner-solver iterative-schur|assembled-dense|woodbury-exact\n";
            std::cout << "  --optimal-port-inner-refinement-max-iters N  outer exact-Schur refinement (default 3)\n";
            std::cout << "  --optimal-port-inner-refinement-tol VALUE   outer refinement target (default 1e-10)\n";
            std::cout << "      legacy aliases auto|direct|pcg|fgmres remain supported\n";
            std::cout << "  --optimal-port-inner-tol x --optimal-port-inner-max-iters N\n";
            std::cout << "  --optimal-port-eigen-tol x --optimal-port-eigen-max-iters N\n";
            std::cout << "  --optimal-port-ablation constant-geometry|input-particular-only|trace-transfer-only|generalized-transfer-only|constant-geometry-trace|constant-geometry-generalized|original-mandatory-trace\n";
            std::cout << "  --optimal-port-source-mode trace-only|trace-plus-input|generalized-dynamic\n";
            std::cout << "  --optimal-port-topology-audit   write source/target audit only; no eigensolve/transient\n";
            std::cout << "  --optimal-port-basis-pilot      min/median/max-interface rank 8/12/16 pilot; no transient\n";
            std::cout << "  --optimal-port-target-solver-comparison  compare three exact target paths; no transient\n";
            std::cout << "  --optimal-port-woodbury-pilot   interface-16 Woodbury gate only; no transient\n";
            std::cout << "  --optimal-port-refinement-validation  compare Woodbury refinement off/on; no transient\n";
            std::cout << "  --optimal-port-maximum-interface-refinement-pilot  audited maximum interface only; no transient\n";
            std::cout << "  --optimal-port-representative-interface-pilot  audit-selected min/median/max rank-4/8 basis gate; no transient\n";
            std::cout << "  --optimal-port-all-interface-basis  explicit streaming all-interface rank-4 basis build; no transient\n";
            std::cout << "  --optimal-port-topology-audit-csv FILE  required source of representative/all-interface selection\n";
            std::cout << "  --residual-krylov-max-rank 0|1|2|4|8  accepted enrichment directions per interface\n";
            std::cout << "  --residual-krylov-max-sweeps 1|2|3 --residual-krylov-tol VALUE\n";
            std::cout << "  --residual-krylov-block-size N --residual-krylov-probe-mode operator-geometry|particular-only\n";
            std::cout << "  --residual-krylov-inner-solver woodbury-exact\n";
            std::cout << "  --residual-krylov-representative-pilot  min/median/max interfaces only; no transient\n";
            std::cout << "  --residual-krylov-all-interface-basis  explicit stage-C build; no transient\n";
            std::cout << "  --randomized-port-rank 8|16|32 --randomized-oversampling N (default 5)\n";
            std::cout << "  --randomized-power-iterations 0|1 --randomized-seed N (default 12345)\n";
            std::cout << "  --randomized-port-compare-optimal  build same-rank Optimal reference for subspace diagnostics\n";
            std::cout << "  --randomized-port-representative-pilot  audit-selected min/median/max interfaces only; no transient\n";
            std::cout << "  --history-compression-method none|deterministic-rrqr  algebraic Local Block Arnoldi history-source compression\n";
            std::cout << "  --history-compression-rank 16|32|64|128|256  compressed history rank (default 0 when disabled)\n";
            std::cout << "  --history-compression-tolerance VALUE  positive rank-revealing tolerance (default 1e-12)\n";
            std::cout << "  --history-compression-max-interface-pilot  audited maximum interface only; no transient\n";
            std::cout << "  --milestone8-production-basis-only  serialize the complete fixed M8 hybrid basis and stop before Schur/transient\n";
            std::cout << "  --milestone8-adaptive-production  apply the frozen M8.8 per-interface rank policy for M8.9 generation/load\n";
            std::cout << "  --adaptive-port-local-pilot --adaptive-port-interface-ids A,B,...  selected-interface hybrid rank pilot; no transient/FOM\n";
            std::cout << "  --global-interface-coarse-prototype --global-interface-coarse-inverse-mode schur-jacobi|exact-pcg --global-interface-coarse-rank 4|8|16|32 --global-interface-coarse-interface-ids A,B,...  M8.10 projected coarse prototype\n";
            std::cout << "  --global-randomized-schur --global-randomized-rank N --global-randomized-composition global-only|augment-local --global-randomized-seed N  M8.11 matrix-free global range finder\n";
            std::cout << "  --milestone8-projection-diagnosis --projection-interface-ids A,B,...  M8.12 one-step full-interface projection diagnosis\n";
            std::cout << "  --milestone8-flux-operator-audit --projection-interface-ids A,B,...  M8.13 homogeneous/affine SIPG flux audit\n";
            std::cout << "  --flux-aware-port --flux-aware-flux-type numerical|physical|both  M8.13 joint-metric randomized port prototype\n";
            std::cout << "  --mor-arnoldi-moments n       block Krylov moment count (default 2)\n";
            std::cout << "  --mor-arnoldi-expansion-point s0  Stage 2C.1 requires s0=0\n";
            std::cout << "  --mor-arnoldi-rank-tolerance tol  rank-revealing block QR threshold\n";
            std::cout << "  --mor-arnoldi-second-moment-energy e  retain C*q energy fraction in the second moment (default 1)\n";
            std::cout << "  --mor-arnoldi-second-moment-max-columns n  cap selected second-moment RHS columns (0 disables)\n";
            std::cout << "  --mor-transient-mass consistent|lumped  capacity discretization\n";
            std::cout << "  --mor-transient-dt/t-end value  transient time grid\n";
            std::cout << "  --mor-transient-integrator backward-euler|crank-nicolson\n";
            ...
            std::cout << "  --transient-local-rom-max-steps N  optional bounded validation smoke test\n";
            std::cout << "  --local-mor-generate           build independently trained local interior POD/RB bases\n";
            std::cout << "  --local-mor-load/save path     load or save a Local-ROM-Schur model (serialization follows the Milestone 1 gate)\n";
            std::cout << "  --local-mor-method pod|rb      local steady basis construction method\n";
            std::cout << "  --local-mor-mode/--local-rom-mode pure|corrected  pure local ROM or exact Stage-1 Schur correction\n";
            std::cout << "  --local-mor-rank r             common fixed local rank; 0 selects independently by tolerance\n";
            std::cout << "  --local-mor-rank-per-subdomain csv  one explicit rank per physical subdomain\n";
            std::cout << "  --local-mor-rank-file path     CSV with subdomain,rank for every physical subdomain\n";
            std::cout << "  --local-mor-energy-tolerance tol  per-subdomain discarded POD energy\n";
            std::cout << "  --local-mor-singular-value-tolerance tol  per-subdomain numerical-rank threshold\n";
            std::cout << "  --local-mor-training-cases n   deterministic FOM training-case count\n";
            std::cout << "  --local-mor-validation-cases n deterministic held-out validation-case count\n";
            std::cout << "  --local-mor-test-cases n       deterministic held-out test-case count\n";
            std::cout << "  --local-mor-matrix-free-threshold n  use Local-ROM FGMRES above n interface DOFs (default 20000)\n";
            std::cout << "  --local-interface-mode full|reduced  Milestone 1 requires the full physical interface\n";
            std::cout << "  --local-rom-reuse-identical-subdomains  reserve identical-subdomain template reuse\n";
            std::cout << "  Stage 2A.1 is fixed-matrix only. Stage 2B.1 supports exactly one active matrix parameter; models are invalid after mesh, material/boundary, interface, source, or affine-component changes.\n";
            ...
            std::cout << "  --solvers p2-p1-aux-pcg          inherited P1 auxiliary-space PCG prototype\n";
                             ...
                             const CaseConfig& physics)
{
    ...
    out << "model_name,config_path,output_dir,solver_method,schwarz_type,schwarz_standalone_mode,"
        ...
        << "schwarz_validate_against_monolithic,interface_mode,penalty_mode,penalty_factor,penalty_scaling,"
...
static void initializeAssemblyDiagnostics(int n, AssemblyDiagnostics& diagnostics)
{
    ...
    diagnostics.interfaceTraceMassDiag.assign(static_cast<size_t>(n), 0.0);
                                                    ...
                                                    size_t faceIndex)
{
    ...
    const SipgInterfaceScales scales = sipgInterfaceScales(
        config, kLeftNormal, kRightNormal, hLeft, hRight);
            ...
            for (int aTest = 0; aTest < 10; ++aTest) {
                ...
                for (int bTrial = 0; bTrial < 10; ++bTrial) {
                    ...
                    local.consistency[static_cast<size_t>(aTest * 20 + bTrial)] +=
                        (-scales.weightLeft * dUL * vL
                         + adjointSign * scales.weightLeft * dVL * uL) * weight;
                    local.consistency[static_cast<size_t>(aTest * 20 + 10 + bTrial)] +=
                        (-scales.weightRight * dUR * vL
                         - adjointSign * scales.weightLeft * dVL * uR) * weight;
                    local.consistency[static_cast<size_t>((10 + aTest) * 20 + bTrial)] +=
                        (scales.weightLeft * dUL * vR
                         + adjointSign * scales.weightRight * dVR * uL) * weight;
                    local.consistency[static_cast<size_t>((10 + aTest) * 20 + 10 + bTrial)] +=
                        (scales.weightRight * dUR * vR
                         - adjointSign * scales.weightRight * dVR * uR) * weight;
