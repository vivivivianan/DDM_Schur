...
{
    // Preserve the validated QR/MGS path bit-for-bit for Euclidean M3.
    // Weighted metrics use two-pass modified Gram-Schmidt with u^T G v.
    if (metric != nullptr) {
        OrthonormalBlock weighted;
        auto inner = [&](const double* u, const double* v) {
            std::vector<double> image = metric->multiply(
                std::vector<double>(v, v + rows));
            return dotColumns(u, image.data(), rows);
        };
        double scale = 0.0;
        for (int j = 0; j < columns; ++j) {
            const double* v = candidates.data() + static_cast<std::size_t>(j) * rows;
            scale = std::max(scale, std::sqrt(std::max(0.0, inner(v, v))));
        }
        const double threshold = tolerance * std::max(1.0e-300, scale);
        for (int j = 0; j < columns; ++j) {
            std::vector<double> v(candidates.begin() + static_cast<std::ptrdiff_t>(j * rows),
                candidates.begin() + static_cast<std::ptrdiff_t>((j + 1) * rows));
            for (int pass = 0; pass < 2; ++pass) {
                for (int q = 0; q < existingColumns; ++q) {
                    const double* basis = existing.data() + static_cast<std::size_t>(q) * rows;
                    axpyColumn(-inner(basis, v.data()), basis, v.data(), rows);
                }
                for (int q = 0; q < weighted.columns; ++q) {
                    const double* basis = weighted.values.data() + static_cast<std::size_t>(q) * rows;
                    axpyColumn(-inner(basis, v.data()), basis, v.data(), rows);
                }
            }
            const double norm = std::sqrt(std::max(0.0, inner(v.data(), v.data())));
            if (!(norm > threshold) || !std::isfinite(norm)) { ++weighted.deflated; continue; }
            scaleColumn(1.0 / norm, v.data(), rows);
            weighted.values.insert(weighted.values.end(), v.begin(), v.end());
            ++weighted.columns;
        }
        weighted.residual = 0.0;
        weighted.orthogonalityError = 0.0;
        return weighted;
    }
    ...
    const std::string& orthogonalization)
{
    ...
    if (basisType != "standard" && basisType != "shift-invert") {
        throw std::runtime_error(
            "Block Arnoldi basis type must be standard or shift-invert.");
    }
    if (orthogonalization != "euclidean" && orthogonalization != "mass"
        && orthogonalization != "k-energy") {
        throw std::runtime_error(
            "Block Arnoldi orthogonalization must be euclidean, mass, or k-energy.");
    }
    if (basisType == "shift-invert"
        && (!(shift > 0.0) || !std::isfinite(shift))) {
        throw std::runtime_error(
            "Shift-invert Block Arnoldi requires a finite positive shift.");
    }
    ...
    const SparseMatrix* orthogonalityMetric = nullptr;
    if (orthogonalization == "mass") orthogonalityMetric = &system.capacity;
    if (orthogonalization == "k-energy") orthogonalityMetric = &system.conductivity;
    result.shift = basisType == "shift-invert" ? shift : 0.0;
    result.rankTolerance = rankTolerance;

    // Factor K_II (or K_II+sigma M_II) once and reuse it for every block
    // moment. The caller controls
    ...
    SparseMatrix shifted(system.dofs);
    shifted.appendScaledEntries(system.conductivity, 1.0);
    if (basisType == "shift-invert") {
        shifted.appendScaledEntries(system.capacity, shift);
    }
    shifted.finalizeCsr();
    const std::vector<MatrixEntry> entries = sparseMatrixEntries(shifted);
    ...
    OrthonormalBlock compressedInput = rankRevealingMgs(
        ...
        noExistingBasis, 0, rankTolerance * 1.0e-2, orthogonalityMetric);
    ...
    OrthonormalBlock block = rankRevealingMgs(
        ...
        result.basis, 0, rankTolerance, orthogonalityMetric);
    ...
    for (int moment = 2; moment <= moments && previousBlockColumns > 0; ++moment) {
        ...
        block = rankRevealingMgs(
            ...
            result.basis, result.rank, rankTolerance, orthogonalityMetric);
