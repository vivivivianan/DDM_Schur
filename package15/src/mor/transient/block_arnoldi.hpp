...
struct BlockArnoldiResult {
    ...
    // "standard": K^{-1} then K^{-1}M Krylov recurrence.
    // "shift-invert": (K+sigma M)^{-1} then (K+sigma M)^{-1}M.
    std::string basisType = "standard";
    // euclidean: V^T V=I; mass: V^T C V=I; k-energy: V^T K V=I.
    std::string orthogonalization = "euclidean";
    double shift = 0.0;
...
BlockArnoldiResult buildBlockArnoldiBasis(
    ...
    const std::string& basisType = "standard",
    double shift = 0.0,
    const std::string& orthogonalization = "euclidean");
